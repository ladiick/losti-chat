...
export const peopleSlice = createSlice({
  ...
  reducers: {
    ...
    updatePeople(state, action) {
      const arr1 = [action.payload.data.sender.pk, action.payload.data.recip.pk].sort();
      const peopleIndex = state.people.findIndex((obj) => {
        const arr2 = [obj.sender.pk, obj.recip.pk].sort();
...
export const { setCurrentPeople, updatePeople, setIndex, setAddFriend, addPeople } =
  peopleSlice.actions;
